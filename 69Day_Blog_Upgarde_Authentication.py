...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash("The email already exist. Please login")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        email_filled = form.email.data
        password_filled = form.password.data
        user = db.session.execute(db.select(User).where(User.email == email_filled)).scalar()
        ...
        elif check_password_hash(user.password, password_filled):
            login_user(user)
            return redirect(url_for("get_all_posts"))
...
def show_post(post_id):
    ...
    all_comments = db.session.execute(db.select(Comments)).scalars().all()
    ...
    return render_template("post.html", image_url=image_url, all_comments=all_comments, post=requested_post, form=form, current_user=current_user)
